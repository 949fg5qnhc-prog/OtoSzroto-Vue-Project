...
const loadListing = async () => {
  try {
    const { data } = await axios.get(`http://localhost:5001/listings/${listID}`);
    Object.assign(form, data); 
  } catch (error) {
    console.error('Błąd w pobieraniu ogłoszenia:', error);
    toast.error('Nie można wczytać ogłoszenia');
  }
};
onMounted(() => {
    loadListing();
...
const handleSubmit = async () => {
    ...
    try {
        const response = await axios.put('http://localhost:5001/listings', newListing);
        router.push(`/listings/${response.data.id}`);
        toast.success('Dodano ogłoszenie!');
    } catch (error) {
        console.error('Błąd edytowania ogłoszenia: ', error);
        toast.error('Błąd w edytowaniu ogłoszenia');
    }
};
